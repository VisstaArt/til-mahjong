...
const CATEGORIES = [
  ...
  "Профессии", "Транспорт и вождение", "Спорт и игры", "Эмоции и чувства",
  ...
  "Предметы и вещи", "Служебные слова",
  "Птицы", "Насекомые", "Цветы и растения", "Геометрические фигуры",
  "Водитель", "Косметика", "Офис", "Здоровье", "Бизнес",
];
// "Служебные слова" — только для того, что физически нельзя отнести ни к одной
// теме (вопросительные слова, местоимения, союзы). Конкретные предметы, которые
// не подошли ни под одну тематическую категорию, идут в "Предметы и вещи" —
// это отдельная категория, а не мусорная корзина для всего подряд.
const FALLBACK_CATEGORY = "Служебные слова";
...
async function translateAndDescribe(words, apiKey) {
  const prompt =
    ...
    `Если слово — глагол, прилагательное, наречие или числительное, СНАЧАЛА проверь, не подходит ли оно ` +
    `под соответствующую категорию из списка ("Глаголы — действия", "Прилагательные и сравнения", ` +
    `"Числительные" и т.п.) — используй именно её, а не "Предметы и вещи" или "${FALLBACK_CATEGORY}". ` +
    `"Предметы и вещи" — для конкретных существительных-предметов, которые не подошли ни под одну ` +
    `тематическую категорию (зонт, компьютер, ножницы и т.п.). "${FALLBACK_CATEGORY}" — только для того, ` +
    `что физически не про предмет/действие/признак (вопросительное слово, местоимение, союз, частица). ` +
    `Учитывай узкие категории раньше широких: птица → "Птицы" (не "Животные"), насекомое → "Насекомые" ` +
    `(не "Животные"), цветок/растение → "Цветы и растения" (не "Природа и погода"), геометрическая фигура ` +
    `→ "Геометрические фигуры", слово про вождение/дорогу/автомобиль в процессе езды → "Водитель" ` +
    `(марки/виды транспорта — в "Транспорт и вождение"), декоративная косметика → "Косметика" ` +
    `(не "Ванная и гигиена" и не "Предметы и вещи"), офисная техника/канцелярия для взрослых → "Офис" ` +
    `(школьные принадлежности для детей — в "Школа и канцтовары"), здоровье/болезни/лечение/самочувствие ` +
    `→ "Здоровье" (не "Предметы и вещи" и не "Прилагательные и сравнения"). ` +
...
async function reclassifyAllWords(onProgress) {
  ...
  for (let i = 0; i < all.length; i += CHUNK_SIZE) {
    ...
    const prompt =
      ...
      `${CATEGORIES.map((c) => `"${c}"`).join(", ")}. Если слово — глагол/прилагательное/наречие/` +
      `числительное, СНАЧАЛА проверь подходящую категорию по части речи ("Глаголы — действия" и т.п.), ` +
      `а не "Предметы и вещи"/"${FALLBACK_CATEGORY}". "Предметы и вещи" — для конкретных предметов без ` +
      `подходящей темы. "${FALLBACK_CATEGORY}" — только для вопросительных слов, местоимений, союзов, частиц. ` +
      `Узкие категории — раньше широких: птица → "Птицы", насекомое → "Насекомые", цветок/растение → ` +
      `"Цветы и растения", геометрическая фигура → "Геометрические фигуры", вождение/дорога/автомобиль ` +
      `в процессе езды → "Водитель" (виды транспорта — в "Транспорт и вождение"), декоративная косметика ` +
      `→ "Косметика", офисная техника/канцелярия для взрослых → "Офис" (школьные — "Школа и канцтовары"), ` +
      `здоровье/болезни/лечение/самочувствие → "Здоровье". ` +
